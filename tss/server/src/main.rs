...
//refcell, arc
#[post("/signupaddshare", format = "json")]
fn signup_addshare(db_mtx: State<RwLock<HashMap<Key, String>>>) -> Json<Result<PartySignup, ()>>
{
    let data = fs::read_to_string("params.json")
        .expect("Unable to read params, make sure config file is present in the same folder ");
    let params: Params = serde_json::from_str(&data).unwrap();
    let threshold = params.threshold.parse::<u16>().unwrap()+1;
    let key = "signup-addshare".to_string();
...
fn main() {
    ...
    let party_signup_addshare = PartySignup {
        number: party1,
        uuid: uuid_addshare
    };
    ...
    };
    {
        ...
        hm.insert(addshare_key, serde_json::to_string(&party_signup_addshare).unwrap());
        hm.insert(reshare_key, serde_json::to_string(&party_signup_reshare).unwrap());
    }
    /////////////////////////////////////////////////////////////////
    rocket::ignite()
        .mount("/", routes![get, set, signup_keygen, signup_sign, signup_addshare,signup_reshare])
        .manage(db_mtx)
        .launch();
}
