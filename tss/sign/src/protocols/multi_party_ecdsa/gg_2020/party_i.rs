...
fn map_share_to_new_param(index: usize, s:&[usize], bks: &[u32], share_count:usize)-> FE {
    let s_len = s.len();

    let points: Vec<FE> = (0..share_count).map(|i|{
        let index_bn = BigInt::from(bks[i]);
...
impl SignKeys {
    ...
    pub fn create(
        ...
        let len = bks.clone().len();
        let li = map_share_to_new_param(index, s,&bks, len);
        let w_i = li * x_i;
        let g: GE = ECPoint::generator();
        let g_w_i = g * w_i;
